...
Now in the above Eq.\eqref{eq:tfimpsx} to find out the drive parameter, at which $\hat{\mathcal{H}}_{_{TFIM+S_{x}}}^{R W A}$ becomes zero, is difficult. This is because it consists of two different Bessel functions simultaneously. So when the first term of the above equation is localized at roots of $\mathcal{J}_0\left(\frac{2h}{\omega}\right)$, the second term of the RWA-equation remains present. Thus effective RWA-Hamiltonian reduces to the second part of Eq.\eqref{eq:tfimpsx}, then its eigen states are in representation $S^x$ identity. So, IPR in $S^x$ representation is unity. 

The second term incorporates a set of non-interacting spins, which assert a localization. Nevertheless, the localization exhibited in the second term originates from the basis of the interaction term, which is along the `x-axis'. So, it is evident from the RWA equation that even though at the CDT/DL point regarded as the transverse field basis, the system cannot be localized, there is certainty of localization at the spin-spin interaction basis.  
