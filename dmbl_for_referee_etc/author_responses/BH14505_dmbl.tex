...
\begin{document}
	...
	\begin{enumerate}
		...
		\response{
			...
			the focus has been on determining the onset of DMBL by looking at expectation values of local
			observable(s) . Standard quantities that have been observed include magnetization, heating rate, and fidelity susceptibility. The observables were computed by running simulations over an extended duration, and localization was inferred from their dynamics over long but ultimately finite times.
			
			The novelty in our paper lies in looking for DMBL in the Floquet qusaistationary states themselves, rather than the observable dynamics. Localization in the Floquet modes can be inferred from their Inverse Participation Ratio, which distinguishes between a fully localized state and a thermal state of the system. IPR ranges from scaling inversely with the Hilbert space dimension (a fully distributed or thermal state) to unity (a fully localized state). Our perspective is that this is a better indicator of localization, as it is valid for infinite times, as opposed to observable dynamics, where very long time effects are onconclusive. We have updated our introduction to elaboate on this.
		}
