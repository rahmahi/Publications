...
\begin{document}
	...
	\begin{enumerate}
		...
		\begin{figure}[h!]
			...
			\caption{Temporal average of Spin-Spin correlation ($\expval{S^z_i S^z_j}$) for different $(2h/\omega)$, between two spins of a 1D spin-1/2 chain. The  vertical dashed lines are the roots of Bessel function($\mathcal{J}_0$). The spin correlation is suppressed at lower ($2h/\omega$) ratio, however it tends to attain unity at higher roots of $\mathcal{J}_0$.}
			\label{fig:TFIM_sx}
		\end{figure}

		We have plotted the long time averaged spin-spin correlation $\expval{S^z_i S^z_{i+1}}$ between two spins of a 1D spin chain (considering periodic boundary condition) for different values of drive parameter ratio $(2h/\omega)$. At lower $(2h/\omega)$ values,$\expval{S^z_i S^z_{i+1}}$ is found to be suppressed. As the ratio is found to rise gradually, the $\expval{S^z_i S^z_{i+1}}$ is also found to rise to unity for all peaks. This corroborates the approximation to recover the dynamical localization at points corresponding to higher roots of $\mathcal{J}_0$.
		}
