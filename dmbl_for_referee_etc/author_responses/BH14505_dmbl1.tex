...
\begin{document}
	...
	\begin{enumerate}
		...
		\response{
			...
			\begin{equation}
				\tilde{H}(t)^{RWA}_{_{XXZ}}= \frac12\Bigg[\sum_i J\left( \hat{\sigma}^x_i \hat{\sigma}^x_{i+1} + \hat{\sigma}^y_i \hat{\sigma}^y_{i+1} \right) + \Delta \left(\sum_{i}  \hat{\sigma}_{i}^{z} \hat{\sigma}_{i+1}^{z}\right)\Bigg].
				\label{eq:finalrwa}
			\end{equation}
			Now we apply Jordan Wigner transformation on the first two terms in Eq.~\eqref{eq:finalrwa}, which reduces the RWA Hamiltonian to the tight-binding Hamiltonian, 
			\begin{equation}
				...
				\label{eq:jwa}
			\end{equation}
		\todo[inline]{This is incomplete. YOU HAVE TO DO TRANSFORMATION ON THE $\sigma^z_i\sigma^z_{i+1}$ in eq~\ref{eq:jwa}.}
			The third term of Eq. \eqref{eq:jwa} commutes with the transverse field component, which keeps its contribution to dynamics invariant. The first two terms represent localization due to the presence of N local (approximate) Noether charges, given by $\hat{n}_m= \hat{d}^{\dagger}_m\hat{d}^{\;}_m$, where
			\begin{equation}
				\hat{d}_m = \frac{1}{\sqrt{N}}\sum_{n=0}^{N-1}e^{-\frac{2nm\pi i}{N}}\hat{c}_n.
			\end{equation}
			When the spin-spin interaction is beyond the nearest-neighbor limit, the Jordan-Wigner transformation is invalid. The Eq. \eqref{eq:rwa3} becomes,
			\begin{align}
				...
				\label{eq:rwalong}
			\end{align}
			However, for each term in Eq.~\eqref{eq:rwalong} the transverse field dynamics is frozen when $2h/\omega$ at one of the roots of $\mathcal{J}_0$, since, $\displaystyle \comm{\tilde{H}(t)^{RWA}_{_{XXZ}}}{\displaystyle\sum_m\sigma^z_m}=0$. 
			\begin{figure}[t]
				...
				\label{fig:std_Ns}
			\end{figure}
			We have numerically simulated the spin-spin correlation $\expval{S^z_i S^z_j}$ $(i=0,j=3)$ for a spin-1/2 nearest-neighbour XXZ spin model comprising of eight spins driven at high frequency $\omega=90$. The system is driven by a transverse sinusoidal drive with high frequency $\omega =90$ and amplitude corresponding to the ratio $2h/\omega$. We found in Fig.\ref{fig:std_Ns} that $\expval{S^z_i S^z_j}$ is unity when $2h/\omega$ lies at the roots of $\mathcal{J}_0$. This supports the application of our localization method in the XXZ model.
			
			However, if an additional integrability-breaking term such as $\sum_i\hat{\sigma}^x_i$ is added to the XXZ Hamiltonian in Eq.~\eqref{eq:heisenberg_model} it becomes,
			\begin{equation}
			...
			\begin{equation}
				...
				\label{eq:jwaxxzsx}
			\end{equation}
			The first two terms of the above equation represents localization in the system, though the third term does not. Now if we consider the asymptotic form of the Bessel function, we can put the discussion in a similar manner in Comment no. 3, then, at the higher root of $\mathcal{J}_0$, the term $\displaystyle \frac12 \mathcal{J}_{0}\left(\frac{h}{\omega}\right) \sum_i\hat{\sigma}^x_i$ contributes very little. 
			\begin{figure}[t]
				\includegraphics[height=5cm]{xxzsx.png}
				\caption{Long time average of spin-spin correlation $\expval{S^z_i S^z_{j}}$ between $i=0$ and $i=3$ at different ($2h/\omega$) for XXZ model along with additional integrability breaking term $\sum_i \hat{\sigma}^x_i$. It consists of eight spins. The drive frequency is kept constant at $\omega = 90$. Two different anisotropies $\Delta = 0.5$ (left panel,$A$), $1.5$ (right panel, $B$) applied. The spin-spin correlation was found to be in unity at each of the roots (vertical gray dashed lines) of the Bessel function in both of the panels.}
				\label{fig:xxzsx}
			\end{figure}
			
			We have simulated the Hamiltonian in Eq.~\eqref{eq:xxz2} to numerically calculate the time-averaged spin-spin correlation $\expval{S^z_i S^z_j}$. The system is taken $N=8$, drive frequency $\omega=90$ and correlation sites $i=0$ and $j=3$. The spin correlation $\expval{S^z_i S^z_j}$ is found to be suppressed at the lower roots of $\mathcal{J}_0$, however, at the higher roots it is found to be unity. This method can be applicable to observe localization in the XXZ model for spin ranges beyond nearest-neighbor interaction. Therefore, it is possible to achieve a localization in the XXZ model even in presence of a longitudinal integrability term, although we have to choose the drive parameters such that $2h/\omega$ lies at higher roots of $\mathcal{J}_0$. 
			
			\todo[inline]{Detail the changes to figure panels etc. suggested by the referee.}
