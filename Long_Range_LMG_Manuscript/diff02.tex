...
%DIF DEL main_110823.tex   Wed Mar  6 10:59:19 2024
%DIF ADD main.tex          Wed Mar  6 10:59:19 2024
reprint,
superscriptaddress,
%linenumbers,
amsmath,amssymb,
...
\DIFdelend Under certain resonance conditions in the drive parameters, periodically driven quantum many-body systems can experience dynamical many-body freezing (DMF), which causes the response to freeze completely to its initial value at all times\DIFdelbegin %DIFDELCMD < }%%%
...
In this article, {we use the fact that emergent approximate symmetries can be engineered in Floquet systems~\cite{Engelhardt2013,asmi:scars} and apply it to long-range interactions This results in \textit{Dynamical Many Body Localization} (DMBL) at resonant values of the drive parameters}, and complete thermal \DIFdelbegin \DIFdel{behaviour }\DIFdelend \DIFaddbegin \DIFadd{behavior }\DIFaddend at other values. This phenomenon is distinct from DMF in the TFIM, since clean TFIM systems, being integrable, never thermalize.

To demonstrate the onset of MBL, we investigate the driven Lipkin-Meshkov-Glick (LMG) model\cite{lmg1965_1,lmg1965_2, lmg1965_3, debergh_2001, ribeiro2008,Engelhardt2013,titum2020}, a long-range system \DIFdelbegin %DIFDELCMD < {%%%
...
\DIFdelend As we move away from the resonance point, the full IPR decreases \DIFdelbegin %DIFDELCMD < {%%%
...
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=9.3cm]{figure_6.jpeg}
	\DIFaddendFL \caption{Temporal average of $\expval{\hat{S^x}}$ (\DIFdelbeginFL \DIFdelFL{ordinate}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{y-coordinate}\DIFaddendFL ) for different $\eta$'s (\DIFdelbeginFL \DIFdelFL{abscissa}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{x-coordinate}\DIFaddendFL ) is plotted for $\sim 200T$ at higher $\omega$ for different N=10, 20, 50. $\overline{\langle S^x \rangle}$ is found to be unity at roots of $J_0(\eta)$. At points away from resonance points, $\expval{\hat{S^x}}$ falls below unity. The corresponding standard deviation $\expval{\hat{S^x}}_{std}$ supports the variation of $\overline{\langle S^x \rangle}$ (inset \DIFdelbeginFL \DIFdelFL{fig.}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{figure}\DIFaddendFL ). $\expval{\hat{S^x}}_{std}$ is $\sim 0$ describing a full freezing of the system at roots of $\mathcal{J}_0(\eta)$ (red vertical solid lines).}
