...
%DIF DEL main_110823.tex   Wed Mar  6 17:48:35 2024
%DIF ADD main.tex          Thu Mar  7 10:51:35 2024
reprint,
superscriptaddress,
%linenumbers,
amsmath,amssymb,
...
%DIFDELCMD < {%%%
\DIFdel{Under certain resonance }\DIFdelend \DIFaddbegin \DIFadd{In the last few years, periodically driven quantum many-body systems have been of considerable theoretical and experimental interest~\mbox{%DIFAUXCMD
\cite{bordia_periodically_2017, sahoo_periodically_2019}}\hspace{0pt}%DIFAUXCMD
. Under certain }\DIFaddend conditions in the drive parameters, \DIFdelbegin \DIFdel{periodically driven quantum many-body systems }\DIFdelend \DIFaddbegin \DIFadd{they }\DIFaddend can experience dynamical many-body freezing (DMF), which causes the response to freeze completely to its initial value at all times\DIFdelbegin %DIFDELCMD < }%%%
\DIFdel{~\mbox{%DIFAUXCMD
\cite{bordia_periodically_2017, sahoo_periodically_2019, das_exotic_2010}}\hspace{0pt}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{~\mbox{%DIFAUXCMD
\cite{das_exotic_2010, sirshendu:freezing, asmi:floquet:thermalization}}\hspace{0pt}%DIFAUXCMD
}\DIFaddend . This arises as a consequence of additional approximate symmetries that occur at \DIFdelbegin \DIFdel{resonance}\DIFdelend \DIFaddbegin \DIFadd{the freezing points}\DIFaddend ~\cite{asmi:scars}. DMF has been demonstrated via the Rotating Wave Approximation (RWA) in the driven TFIM with \DIFdelbegin \DIFdel{nearest-neighbour }\DIFdelend \DIFaddbegin \DIFadd{nearest-neighbor }\DIFaddend interactions ~\cite{mbeng_quantum_2020} and is shown to be protected when translational invariance is explicitly broken (say, by disorder) ~\cite{yamada_localization_2022, roy_fate_2015}. 
...
in typical states\cite{zhang_floquet_2016}. The ETH can be readily adapted to time-periodic systems using Floquet theory (the Floquet-ETH, or FETH \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
...
In this article, {we use the fact that emergent approximate symmetries can be engineered in Floquet systems~\cite{Engelhardt2013,asmi:scars} and apply it to long-range interactions\DIFaddbegin \DIFadd{. }\DIFaddend This results in \textit{Dynamical Many Body Localization} (DMBL) at resonant values of the drive parameters}, and complete thermal \DIFdelbegin \DIFdel{behaviour }\DIFdelend \DIFaddbegin \DIFadd{behavior }\DIFaddend at other values. This phenomenon is distinct from DMF in the TFIM, since clean TFIM systems, being integrable, never thermalize. \DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < 	%%%
\DIFdelend To demonstrate the onset of MBL, we investigate the driven Lipkin-Meshkov-Glick (LMG) model\cite{lmg1965_1,lmg1965_2, lmg1965_3, debergh_2001, ribeiro2008,Engelhardt2013,titum2020}, a long-range system \DIFdelbegin %DIFDELCMD < {%%%
\DIFdelend that extends the \DIFdelbegin \DIFdel{nearest-neighbour }\DIFdelend \DIFaddbegin \DIFadd{nearest-neighbor }\DIFaddend interactions in the TFIM to  all-to-all interactions\DIFdelbegin \DIFdel{.}%DIFDELCMD < }%%%
\DIFdelend ~\cite{campa_statistical_2009, eisele_multiple_1988, canning_class_1992}\DIFaddbegin \DIFadd{. Previous works on driven LMG models have either focused on the low-frequency regime where FETH is applicable~\mbox{%DIFAUXCMD
\cite{russomanno_thermalization_2015}}\hspace{0pt}%DIFAUXCMD
, or the onset of localization inferred from observable dynamics~\mbox{%DIFAUXCMD
\cite{Engelhardt2013, lmg:fidelity, Russomanno2017}}\hspace{0pt}%DIFAUXCMD
.  }\DIFaddend We have recovered the onset of \DIFdelbegin \DIFdel{DMF }\DIFdelend \DIFaddbegin \DIFadd{DMBL }\DIFaddend in this system \DIFdelbegin \DIFdel{and have supported our result with numerical simulations}\DIFdelend \DIFaddbegin \DIFadd{by inspecting the values of the high-frequency Inverse Participation Ratio (IPR) of the Floquet modes themselves, a result that is valid for infinite times, thus being a better indicator of localization}\DIFaddend .

In addition, we compare the degree of localization of the quasi-stationary Floquet modes \DIFdelbegin %DIFDELCMD < {%%%
\DIFdelend in the LMG model with the TFIM\DIFdelbegin %DIFDELCMD < }%%%
\DIFdelend \DIFaddbegin \DIFadd{, as well as other short-range models with lower symmetry}\DIFaddend . In order to do so, we look at the Inverse Participation Ratio (IPR) of the Floquet modes in the representation given by the eigenstates of the symmetry-breaking field. The IPR, closely related to the concept of quantum purity, is defined as the formal sum of the square of the density in some physically meaningful space or representation. A high IPR of a stationary state denotes low participation in most of the representation, and a low IPR distributes participation uniformly across the representation, leading to ergodic dynamics\cite{vu_fermionic_2022}. Thus, IPR~\cite{Misguich2016} is a useful tool for witnessing MBL of a quantum system. For an MBL system, the IPR is unity, and it scales inversely with the {number of spins} when it is thermally distributed~\cite{calixto_inverse_2015}.

In the first section of this paper, we present all essential theoretical frameworks. Our results for the LMG model are presented next in section II. In that section, we have used the \DIFdelbegin \DIFdel{Rotating Wave Approximation (RWA ) }\DIFdelend \DIFaddbegin \DIFadd{RWA }\DIFaddend ~\cite{fujii_introduction_2017}, where only the slowest rotating terms in the Fourier expansion of the Hamiltonian in a frame co-rotating with the symmetry breaking drive field are retained. In addition, we have the obtained {numerical simulations of} the Floquet modes and their IPR. They are used to probe the system dynamics in the high and low-frequency domains at both limits of $\beta$. In section III we have used phase space plots to contrast the low and high frequency limits of the LMG model in the thermodynamic limit by mapping it to an equivalent classical Hamiltonian system. Finally, in section IV, we have looked at numerical computations of the IPR of the Floquet modes for different values of the drive parameters, well beyond those that allow for the RWA. We \DIFdelbegin \DIFdel{observed that, if the system is driven by an adiabatically increasing drive frequency from low to high limit while remaining in the resonance region, a sharp crossover }\DIFdelend \DIFaddbegin \DIFadd{achieved a smooth crossover from a thermal phase to a DMBL phase by increasing the frequency adiabatically while remaining at a freezing point. However, the DMBL phase is unstable at any finite driving frequency in the thermodynamic limit. This result is in contrast to the behavior of short-range models reported in the literature, in which a non-analytic transition }\DIFaddend from a thermal \DIFdelbegin \DIFdel{to an MBL phase occurs. We }\DIFdelend \DIFaddbegin \DIFadd{phase to a strictly local phase takes place at finite driving parameters~\mbox{%DIFAUXCMD
\cite{asmi:floquet:thermalization}}\hspace{0pt}%DIFAUXCMD
. Finally, we }\DIFaddend conclude with discussions and outlook.
...
\DIFdelbegin \DIFdel{This phenomenon is highly general in nature, and can be readily adapted to non-integrable systems. }\DIFdelend \DIFaddbegin \DIFadd{However, this phenomenon is not trivial to generalize, since the addition of integrability-breaking terms, such as longitudinal fields or additional spin-spin interactions, kills the suppression of fermion number dynamics.  For instance, if we apply an integrability-breaking longitudinal field $S_x = \frac{1}{2} \sum_i \hat{\sigma}^x_i$, the Hamiltonian becomes 
}\begin{align}
	\DIFadd{\hat{H}_{_{TFIM+S_{x}}}(t) }&\DIFadd{=\frac{1}{2} \Bigg[\sum^N_{i=1}  \hat{\sigma}_{i}^{x} \hat{\sigma}_{i+1}^{x}+h \cos (\omega t) \sum^N_{i=1} \hat{\sigma}_{i}^{z}\nonumber}\\
	&\DIFadd{\hskip4cm  +\sum^N_{i=1} \hat{\sigma}_{i}^{x}\Bigg].
	\label{eq:tfim_sx}
}\end{align}	
\DIFadd{Performing the unitary transformation and taking the RWA yields a new RWA Hamiltonian
}\begin{equation}
	\DIFadd{\hat{H}_{_{TFIM+S_{x}}}^{R W A}= H^{RWA}+\frac{1}{2} \mathcal{J}_{0}\left(\frac{h}{\omega}\right) \sum_i\hat{\sigma}^x_i.
	\label{eq:tfim_sx1}
}\end{equation}
\DIFadd{At the root of $\mathcal{J}_0\left(\frac{2h}{\omega}\right)$, the longitudinal field survives. However, note that the Bessel function has asymptotic form $\mathcal{J}_0\left(\frac{2h}{\omega}\right)\sim \left(\frac{2h}{\omega}\right)^{-1/2}\cos\left(\frac{2h}{\omega}-\frac{\pi}{4}\right)$, a good approximation for sufficiently  large $h/\omega$. In that limit, if $2h/\omega$ is chosen to lie at a root, then $\mathcal{J}_0\left(\frac{h}{\omega}\right) \sim h^{-1/2}$, which is small for sufficiently large $h$. Thus, the contribution of the longitudinal field is substantially reduced if $h\gg\omega\gg1$, $\mathcal{J}_0\left(\frac{2h}{\omega}\right)=0$, partially recovering dynamical freezing.
}\begin{figure}[t!]
	\centering
	\includegraphics[width =8cm]{figure_1.jpeg}
	\caption{\DIFaddFL{Time-average of spin-correlation $\expval{S^z_0 S^z_3}$ for a driven 1D spin-$1/2$ chain with periodic boundary conditions. The chain consists of $N=8$ spins, and was driven harmonically at fixed frequency $\omega=90$.  The system was initially populated in a fully $z-$polarized spin state and evolved numerically till time $t=300T$, where $T=2\pi/\omega$.  The vertical dashed lines represent the roots of the Bessel function $\mathcal{J}_0$.  Panel-A presents results for  the driven $TFIM+S_x$ (the  Hamiltonian in eq.~\ref{eq:tfim_sx}), and panel-B presents results for the driven  XXZ model (both with and without an $S_x-$field) from the Hamiltonian described in  eq.~\ref{eq:xxzsx}.
...
\end{figure}
\DIFadd{This analysis can be supported by dynamical simulations for small-size systems. We have utilized QuTiP, the Quantum Toolbox in Python~\mbox{%DIFAUXCMD
\cite{qutip}}\hspace{0pt}%DIFAUXCMD
...
\DIFadd{Interestingly, if we apply this approach to investigate the periodically driven XXZ model, we notice that the system remains frozen for all drive parameters. The XXZ Hamiltonian, including a longitudinal field, is given by
}\begin{multline}
	\DIFadd{H_{_{XXZ+S_{X}}} = \frac{1}{2} \sum_{i=1} \bigg[ J \hat{\sigma}^x_i \hat{\sigma}^x_{i+1} +J  \hat{\sigma}^y_i \hat{\sigma}^y_{i+1}}\\ \DIFadd{+ \Delta  \hat{\sigma}^z_i \hat{\sigma}^z_{i+1} + h\cos(\omega t)  \hat{\sigma}^z_i + \hat{\sigma}^x_i\bigg],
	\label{eq:xxzsx}
}\end{multline}
\DIFadd{Transforming to the moving frame  yields
}\begin{equation}
\DIFadd{H_{_{XXZ+S_{x}}}^{mov} =H_{_{XXZ}}^{mov} +  \frac{1}{2} \sum_i\Big[ \hat{\sigma}^x_i \cos(\zeta) + \hat{\sigma}^y_i \sin(\zeta)\Big].
\label{eq:xxzsxmov}
}\end{equation}
\DIFadd{Here, $\zeta \equiv \frac{h}{\omega}\sin(\omega t)$, and the moving frame XXZ Hamiltonian is
}\begin{equation}
\DIFadd{H_{_{XXZ}}^{mov} =  \sum_i\Bigg[J\left(\hat{\sigma}^+_i \hat{\sigma}^-_{i+1} + \hat{\sigma}^-_i \hat{\sigma}^+_{i+1}\right) + \frac{\Delta}{2} \hat{\sigma}^z_i \hat{\sigma}^z_{i+1}\Bigg],
}\end{equation}
\DIFadd{where $\hat{\sigma}^\pm_i$ are spin ladder operators. Note that the moving frame Hamiltonian has no $\zeta-$dependence. If we populate the system initially in a  fully $z-$polarized state, then $H_{_{XXZ}}^{mov}$ does not contribute to the dynamics at all. The remaining longitudinal field  will simply add single particle dynamics to the state. If we now apply RWA in eq.~\ref{eq:xxzsxmov} by smoothing out all harmonic oscillations from the Jacobi -Anger expansion, we get
}\begin{equation}
\DIFadd{H_{_{XXZ+S_{X}}}^{_{RWA}} = H_{_{XXZ}}^{mov} +  \frac{1}{2} \mathcal{J}_0 \left(\frac{h}{\omega}\right)\sum_i \hat{\sigma}^x_i,
\label{eq:xxz_sx_rwa}
}\end{equation}
\DIFadd{Thus, freezing can occur in the XXZ model in the absence of a longitudinal field regardless of the drive parameters. However when the $S_x-$ field is introduced, freezing can occur only when the drive parameters are controlled in such manner that ($h/\omega$) lies on any root of the Bessel function. We have supported this conclusion with QuTiP simulations~\mbox{%DIFAUXCMD
\cite{qutip} }\hspace{0pt}%DIFAUXCMD
similar to those described in the previous paragraph. The resultant spin correlations are plotted against $h/\omega$ for fixed $\omega$ in panel-B of  fig.~\ref{fig:ipr:tfimsx}. 
}

\DIFadd{Now, if the integrability of the TFIM is broken by extending the range of $x-$spin interactions beyond nearest-neighbor, freezing can }\textit{\DIFadd{still}} \DIFadd{be recovered through the emergence of additional approximate conserved quantities at the freezing point (see~\mbox{%DIFAUXCMD
\cite{rahaman2024time} }\hspace{0pt}%DIFAUXCMD
for a detailed derivation). The case of all-to-all interactions is particularly interesting, as complete freezing can be achieved in that limit, as demonstrated in the next section. }\DIFaddend In such cases, freezing has the additional effect of inducing DMBL, suppressing \DIFdelbegin \DIFdel{thermalization }\DIFdelend \DIFaddbegin \DIFadd{Floquet-thermalization }\DIFaddend to infinite temperatures. Numerical quantification of localization of a specific (quasi) stationary state in a physically significant representation can be achieved through the computation of the \emph{Inverse Participation Ratio} (IPR).  {The IPR is generally defined as the formal sum over the square of the local density in a physically meaningful space.}~\cite{mukherjee_modulation-assisted_2015,lin_many-body_2018,murphy_generalized_2011, torres-herrera_self-averaging_2020} {In the single particle case,  the IPR, } for a state $\ket{\psi}$ can be written as
\begin{equation*}
...
\end{equation}
The smallest value of the IPR corresponds to a fully de-localized state, $\psi(x)=1/\sqrt{N}$ for a system of size $N$ \cite{torres-herrera_self-averaging_2020,trivedi_can_2005}. Values of the IPR close to unity correspond to localized states \cite{Misguich2016}. For a periodically driven system, we look at the IPR of the quasi-stationary Floquet modes at $t=T$, where $t=2\pi/\omega$ for drive frequency $\omega$. In the TFIM model, \DIFdelbegin \DIFdel{equation}\DIFdelend \DIFaddbegin \DIFadd{eq.}\DIFaddend ~\ref{eq:hrwa:tfim} indicates that, \DIFdelbegin \DIFdel{at resonance, }\DIFdelend when $J_0(\eta)=0$, the Floquet modes are approximately given by the fermionic Fock states, which have a trivially unit IPR in the representation of the eigenmodes of the transverse field $\hat{H}_1$ in \DIFdelbegin \DIFdel{equation}\DIFdelend \DIFaddbegin \DIFadd{eq.}\DIFaddend ~\ref{eq:tfim:hamilt}. Here, a particular Floquet mode can be decomposed into a direct product of cooper-pair states as $|\phi\rangle = \prod_{k,-k}|\phi^n_k\rangle$. In the RWA limit and at \DIFdelbegin \DIFdel{resonance}\DIFdelend \DIFaddbegin \DIFadd{freezing}\DIFaddend , $|\phi^n_k\rangle$ has values of $|0\rangle, |k,-k\rangle$ for two values of $n=0,1$ respectively. We define the reduced IPR of $|\phi^n_k\rangle\; \forall k$ to be
\begin{equation}
	\label{eq:ipr:ising}
...
\end{equation}
In the RWA limit and at \DIFdelbegin \DIFdel{resonance }\DIFdelend \DIFaddbegin \DIFadd{freezzing }\DIFaddend , this quantity is unity, indicating very low participation and the onset of freezing.
\begin{figure}[t!]
	\centering
	\DIFdelbeginFL %DIFDELCMD < \includegraphics[width = 8.5cm]{ising_exact_ipr_3panel.jpeg}
%DIFDELCMD < 		%%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width = 8.5cm]{figure_2.jpeg}
	\DIFaddendFL \caption{\DIFdelbeginFL %DIFDELCMD < {%%%
\DIFdelendFL Reduced IPR (defined in \DIFdelbeginFL \DIFdelFL{equation}%DIFDELCMD < }%%%
\DIFdelendFL \DIFaddbeginFL \DIFaddFL{eq.}\DIFaddendFL ~\ref{eq:ipr:ising}) {for one of the two Floquet modes obtained from the exact dynamics of the TFIM for size} $N = 100, \omega=90$ {for the entire Brillouin zone (\DIFdelbeginFL \DIFdelFL{top }\DIFdelendFL panel \DIFaddbeginFL \DIFaddFL{A}\DIFaddendFL , \DIFdelbeginFL \DIFdelFL{ordinate}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{y-coordinate}\DIFaddendFL ) and a few drive amplitudes (top panel, \DIFdelbeginFL \DIFdelFL{abscissa}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{x-coordinate}\DIFaddendFL ). The dashed lines (\DIFdelbeginFL \DIFdelFL{top panel}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{in panel-A}\DIFaddendFL ) indicate the roots of} $J_0(\eta)${. The middle panel \DIFaddbeginFL \DIFaddFL{(B) }\DIFaddendFL shows cross-sections of the reduced IPR in} $k-$ {space for four chosen amplitudes. Finally, the bottom panels \DIFaddbeginFL \DIFaddFL{(C) }\DIFaddendFL show semi-log plots of the scaling with} $N$ {of the full many body IPR as defined in \DIFdelbeginFL \DIFdelFL{eqn}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{eq.}\DIFaddendFL }~\DIFdelbeginFL \DIFdelFL{\ref{eq:ipr:manybody}{, with the left panel for a small} }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{\ref{eq:ipr:manybody}{, with the left-C panel for a small} }\DIFaddendFL $\omega=0.4$\DIFdelbeginFL %DIFDELCMD < {%%%
\DIFdelendFL , amplitude \DIFdelbeginFL %DIFDELCMD < } %%%
\DIFdelendFL $h$ {chosen to lie both in and out of the root of} $J_0(\eta)$ {as indicated in the legend, with similar plots on the \DIFdelbeginFL \DIFdelFL{right }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{right-C }\DIFaddendFL panel for a large} $\omega=90$. }
	\label{fig:ipr:tfim}
\end{figure}
Figure~\ref{fig:ipr:tfim} shows results from numerically simulating the TFIM dynamics. The reduced IPR for a particular Floquet mode recovered by simulating the exact Schr\"odinger dynamics over a single time period of the drive, and plotted as a function of momentum $k$ for different $\eta$'s. At \DIFdelbegin \DIFdel{resonance}\DIFdelend \DIFaddbegin \DIFadd{freezing}\DIFaddend , when $\eta$ lies at the root of the Bessel function $J_0(\eta)$,  the {Reduced} IPR is {nearly} unity for all momenta. {Consequently, so is} $\phi_{IPR}$.  Outside \DIFdelbegin \DIFdel{this resonance}\DIFdelend \DIFaddbegin \DIFadd{the freezing point}\DIFaddend , the IPR is unity only for some momenta because the effective Hamiltonian is perfectly diagonal at $k \in{\{-\pi, 0, \pi\}}$ as can be seen in the cross-sectional plots of \DIFdelbegin \DIFdel{figure}\DIFdelend \DIFaddbegin \DIFadd{fig.}\DIFaddend ~\ref{fig:ipr:tfim}. \DIFdelbegin %DIFDELCMD < {%%%
\DIFdelend As we move away from the \DIFdelbegin \DIFdel{resonance }\DIFdelend \DIFaddbegin \DIFadd{freezing }\DIFaddend point, the full IPR decreases \DIFdelbegin %DIFDELCMD < {%%%
\DIFdel{exponentially }%DIFDELCMD < } %%%
\DIFdelend \DIFaddbegin \DIFadd{exponentially }\DIFaddend in the system size, as can be seen in \DIFdelbegin %DIFDELCMD < {%%%
\DIFdelend the bottom-right panel of fig\DIFdelbegin %DIFDELCMD < }%%%
\DIFdel{~\ref{fig:ipr:tfim}}%DIFDELCMD < }
%DIFDELCMD < 	%%%
\DIFdelend \DIFaddbegin \DIFadd{.~\ref{fig:ipr:tfim}.
}\DIFaddend However, as the TFIM is an integrable spin model, the IPR never drops to a value that is small enough to indicate thermalization. \DIFdelbegin %DIFDELCMD < {%%%
\DIFdelend This can be seen in the bottom panels of \DIFdelbegin \DIFdel{figure}%DIFDELCMD < }%%%
\DIFdelend \DIFaddbegin \DIFadd{fig.}\DIFaddend ~\ref{fig:ipr:isinglowfrk}{, where the exponential decay of} $\phi_{IPR}(N)$ {never approaches the thermodynamic scaling law $\phi_{IPR}(N)\sim 2^{-N}$ for either small or large frequencies. Note that, a}t low frequencies, {complete localization} fails due to the \DIFdelbegin \DIFdel{unavailibity }\DIFdelend \DIFaddbegin \DIFadd{unavailability }\DIFaddend of zero off-diagonal terms in the  effective transformed Hamiltonian, as well as the absence of integrability breaking terms to counteract the off diagonal terms. 
\begin{figure}[t!]
	\centering
	\DIFdelbeginFL %DIFDELCMD < \includegraphics[width = 8.5cm]{ising_exact_lowfr_ipr.jpeg}
%DIFDELCMD < 		%%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width = 8.5cm]{figure_3.jpeg}
	\DIFaddendFL \caption{Reduced IPR obtained by adiabatically increasing $\omega$ (top panel \DIFdelbeginFL \DIFdelFL{abscissa}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{x-coordinate}\DIFaddendFL ) for one the floquet mode obtained from \DIFdelbeginFL \DIFdelFL{equation}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{eq.}\DIFaddendFL ~\ref{eq:ipr:ising} at the root of $J_0(\eta)$ for N = 500. IPR is $\sim 0.5$ (localized yet not fully freezing) upto $\omega \sim 2$, after that, smoothly increased to unity (fully localized and freezing) at higher $\omega \geq 10$ (top panel, \DIFdelbeginFL \DIFdelFL{ordinate}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{y-coordinate}\DIFaddendFL ). At bottom panel cross-sections for four chosen amplitudes at $\omega =2$ are plotted for a brillouin zone (\DIFdelbeginFL \DIFdelFL{abscissa}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{x-coordinate}\DIFaddendFL ) with corresponding reduced IPRs (\DIFdelbeginFL \DIFdelFL{ordinate}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{y-coordinate}\DIFaddendFL ).}
...
Here, the undriven part $\hat{H}_0$ and the driven part $\hat{H}_1$ are, respectively, 
	\begin{eqnarray}
		\hat{H}_0 &=& \DIFaddbegin \DIFadd{-}\DIFaddend \frac{2}{N-1} \sum\DIFdelbegin \DIFdel{_{ij}}%DIFDELCMD < \hat{\sigma}%%%
\DIFdelend \DIFaddbegin \DIFadd{_{i<j}S}\DIFaddend ^z_i \DIFdelbegin %DIFDELCMD < \hat{\sigma}%%%
\DIFdelend \DIFaddbegin \DIFadd{S}\DIFaddend ^z_j,\nonumber \\
		\hat{H}_1 &=& \DIFaddbegin \DIFadd{-2 }\DIFaddend \sum\DIFdelbegin \DIFdel{^N_{i=1}}%DIFDELCMD < \hat{\sigma}%%%
\DIFdelend \DIFaddbegin \DIFadd{_i S}\DIFaddend ^x_i.
		\DIFaddbegin \label{eq:h0h1}
	\DIFaddend \end{eqnarray}
	\DIFdelbegin %DIFDELCMD < {%%%
\DIFdelend The Kac-norm of $2/(N-1)$ arises from the choice to maintain the extensivity of the interaction energy\DIFdelbegin %DIFDELCMD < }%%%
...
\begin{figure}[t!]
	\centering
	\DIFdelbeginFL %DIFDELCMD < \includegraphics[width=9.3cm]{wh_phasespace.jpeg }
%DIFDELCMD < 	%%%
...
\begin{figure}[t!]
	\centering
	\DIFdelbeginFL %DIFDELCMD < \includegraphics[width = 8.5cm]{ipr_exact_dynm_N50_frq_90_.jpeg}
%DIFDELCMD < 		%%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width = 8.5cm]{figure_5.jpeg}
	\DIFaddendFL \caption{IPR density plot for all possible Floquet modes (top panel\DIFdelbeginFL \DIFdelFL{ordinate}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{, y-coordinate}\DIFaddendFL ) for different values of $\eta = 4h/\omega$ (top panel \DIFdelbeginFL \DIFdelFL{abscissa}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{x-coordinate}\DIFaddendFL ), deduced from \DIFdelbeginFL \DIFdelFL{equation }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{eq.~}\DIFaddendFL \ref{eq:ipr:lmg} for exact LMG Hamiltonian for N = 50. Blue dashed lines are roots of $J_0(\eta)$. At bottom panel cross-section of IPR (\DIFdelbeginFL \DIFdelFL{ordinate }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{y-coordinate }\DIFaddendFL )for four different $\eta$'s plotted for all possible floquet modes(bottom panel, \DIFdelbeginFL \DIFdelFL{abscissa}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{x-coordinate}\DIFaddendFL ) at $\omega=90$. IPR founds to be $\sim$ unity for all Floquet modes at roots of $J_0$.}
...
The localization of the Floquet states at \DIFdelbegin \DIFdel{resonance }\DIFdelend \DIFaddbegin \DIFadd{freezing }\DIFaddend is supported by exact numerical results, as can be seen in the phase diagram fig\DIFaddbegin \DIFadd{.}\DIFaddend ~\ref{fig:lmg_phasediag}. Here, we have plotted the arithmetic mean over all Floquet states of the IPR 
...
These plots were obtained numerically by diagonalizing the propagator $U(t)$ at $t=T$, where $U(t)$ is defined in \DIFdelbegin \DIFdel{eqn}\DIFdelend \DIFaddbegin \DIFadd{eq.}\DIFaddend ~\ref{eq:propagator}. This propagator was obtained from simulations of the exact quantum dynamics using QuTiP\DIFdelbegin \DIFdel{, the Quantum Toolbox in Python}\DIFdelend ~\cite{qutip}. We kept the frequency at a fairly large value $\omega = 90$ where we expect that RWA would be valid, and $N=\mathcal{O}(10^2)$. The density plot in the upper panel of fig\DIFaddbegin \DIFadd{.}\DIFaddend ~\ref{fig:lmg_ipr_exact} depicts the IPR of the Floquet states; the abscissa \DIFaddbegin \DIFadd{(x-coordinate)  }\DIFaddend $\eta=4h/\omega$ and the ordinate is $n/(2N+1)$, where $n\leq 2N$ is a non-negative integer that indexes the Floquet states in increasing order of $m$. The dashed vertical lines correspond to the roots of $J_0(\eta)$. Comparing with the IPR of the TFIM in fig\DIFaddbegin \DIFadd{.}\DIFaddend ~\ref{fig:ipr:tfim}, we can see a very similar \DIFdelbegin \DIFdel{patterns }\DIFdelend \DIFaddbegin \DIFadd{pattern }\DIFaddend in the immediate \DIFdelbegin \DIFdel{neighbourhood }\DIFdelend \DIFaddbegin \DIFadd{neighborhood }\DIFaddend of the roots. Evidently, the IPR approaches a value of one for sufficiently large values of the roots, strongly suggesting full DMBL. Deviations occur at the smallest root of $J_0(\eta)$ (around $\eta = 2.405$) due to the contributions from higher order terms in eq\DIFaddbegin \DIFadd{.}\DIFaddend ~\ref{eq:lmg_jacobiexp}. Thus, a higher root is favored for DMBL.
...
\begin{figure}[t!]
	\centering
	\DIFdelbeginFL %DIFDELCMD < \includegraphics[width=9.3cm]{sx_conservation.jpeg }
%DIFDELCMD < 		%%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=9.3cm]{figure_6.jpeg}
	\DIFaddendFL \caption{Temporal average of $\expval{\hat{S^x}}$ (\DIFdelbeginFL \DIFdelFL{ordinate}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{y-coordinate}\DIFaddendFL ) for different $\eta$'s (\DIFdelbeginFL \DIFdelFL{abscissa}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{x-coordinate}\DIFaddendFL ) is plotted for $\sim 200T$ at higher $\omega$ for different N=10, 20, 50. $\overline{\langle S^x \rangle}$ is found to be unity at roots of $J_0(\eta)$. At points away from \DIFdelbeginFL \DIFdelFL{resonance }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{these }\DIFaddendFL points, $\expval{\hat{S^x}}$ falls below unity. The corresponding standard deviation $\expval{\hat{S^x}}_{std}$ supports the variation of $\overline{\langle S^x \rangle}$ (inset \DIFdelbeginFL \DIFdelFL{fig.}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{figure}\DIFaddendFL ). $\expval{\hat{S^x}}_{std}$ is $\sim 0$ describing a full freezing of the system at roots of $\mathcal{J}_0(\eta)$ (red vertical solid lines).}
	\label{fig:sx_conserve}
\end{figure}
Small deviations do occur due to the role of  higher order terms in the rotated Hamiltonian in eq\DIFaddbegin \DIFadd{.}\DIFaddend ~\ref{eq:lmg_rotated}. This can be demonstrated quantitatively by comparing the IPR obtained from the exact dynamics simulation with that obtained from the dynamics of $\tilde{H}(t)$ in eq.~\ref{eq:lmg_rotated} after truncating the series at orders $k\geq 1$. This comparison can be seen in fig\DIFaddbegin \DIFadd{.}\DIFaddend ~\ref{fig:lmg_ipr_rwa11}.
\begin{figure}[t!]
	\centering
	\DIFdelbeginFL %DIFDELCMD < \includegraphics[width =9.5cm]{comprasion_LMG_50_highFr90_exact_nd_rwa.jpeg}
%DIFDELCMD < 		%%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width =8.5cm]{figure_7.jpeg}
...
\DIFdelendFL \DIFaddbeginFL \begin{figure}[t!]
	...
	\DIFdelbeginFL %DIFDELCMD < \includegraphics[width = 9.0 cm]{lmg_poincare011.jpeg}
%DIFDELCMD < 		%%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width = 9.0 cm]{figure_8.jpeg}
	\DIFaddendFL \caption{Phase-space distributions at $\omega=1.0$ (\DIFdelbeginFL \DIFdelFL{left }\DIFdelendFL panels \DIFaddbeginFL \DIFaddFL{$X_{a,b}$}\DIFaddendFL ), $\omega=2.5$ (\DIFdelbeginFL \DIFdelFL{middle }\DIFdelendFL panels \DIFaddbeginFL \DIFaddFL{$Y_{a,b}$}\DIFaddendFL ) and $\omega=90.0$ (\DIFdelbeginFL \DIFdelFL{right }\DIFdelendFL panels \DIFaddbeginFL \DIFaddFL{$Z_{a,b}$}\DIFaddendFL ) for $100$ initial conditions. The drive amplitude $h$ is always adjusted such that $\eta=4h/\omega$ lies on the smallest root of $J_0(\eta)$, \textit{i.e.} $\eta=2.4048\dots$. At small $\omega=1.0$, the classical PSOS, obtained from simulating the dynamics in \DIFdelbeginFL \DIFdelFL{eqns}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{eq.}\DIFaddendFL ~\ref{eq:class:poinc} (\DIFdelbeginFL \DIFdelFL{top left }\DIFdelendFL panel \DIFaddbeginFL \DIFaddFL{$X_{a}$}\DIFaddendFL ),  shows chaotic \DIFdelbeginFL \DIFdelFL{behaviour}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{behavior}\DIFaddendFL , and at \DIFaddbeginFL \DIFaddFL{panel $Y_{a}$ where }\DIFaddendFL $\omega = 2.5$, regular regions start to appear. At higher $\omega = 90.0$, the dominance of regular dynamics can be readily seen (\DIFdelbeginFL \DIFdelFL{top right }\DIFdelendFL panel \DIFaddbeginFL \DIFaddFL{$Z_{a}$}\DIFaddendFL ). The bottom panels plot the corresponding Spectral-Averaged Husimi-Q function, obtained from the Floquet modes $\ket{\phi^n}$ using \DIFdelbeginFL \DIFdelFL{eqn}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{eq}\DIFaddendFL .~\ref{eq:husimi}, and setting $N=100$. The $\omega=1.0$ case (\DIFdelbeginFL \DIFdelFL{bottom left }\DIFdelendFL panel \DIFaddbeginFL \DIFaddFL{$X_{b}$}\DIFaddendFL ) has a dispersed distribution in \DIFdelbeginFL \DIFdelFL{colour}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{color}\DIFaddendFL . This is consistent with the chaotic \DIFdelbeginFL \DIFdelFL{behaviour }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{behavior }\DIFaddendFL seen in the continuum limit. At $\omega = 2.5$ (\DIFdelbeginFL \DIFdelFL{bottom middle }\DIFdelendFL panel \DIFaddbeginFL \DIFaddFL{$Y_{b}$}\DIFaddendFL ), there is a partial regular pattern observed together with a dispersed pattern. In the $\omega=90-$case (\DIFdelbeginFL \DIFdelFL{bottom right }\DIFdelendFL panel \DIFaddbeginFL \DIFaddFL{$Z_{b}$}\DIFaddendFL ), the distribution has distinct \DIFdelbeginFL \DIFdelFL{colour }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{color }\DIFaddendFL contrasts, which is consistent with the regular dynamics pattern seen in the continuum limit.}
...
To demonstrate this, we investigate the IPR of the  Floquet mode with smallest quasienergy for numerous frequencies and system sizes, along with the associated drive amplitude $h$ keeping the system at a localization point. The results are shown in fig\DIFaddbegin \DIFadd{.}\DIFaddend ~\ref{fig:phase_crossover}. In the low-frequency range $\omega \in \left[1.0, 9.0\right]$, the IPR exhibits values well below unity.   {Moreover, as can be seen in the bottom panel of the same figure, when the dynamics is simulated for smaller} $\omega$s, {the fall of} $\phi_{IPR}(N)$ {asymptotically approaches one that characterizes a fully thermal state, where} $\phi_{IPR}(N)\sim 1/N$ {in the TSS. } In the limit  $N\rightarrow\infty$, the inverse participation ratio (IPR) tends towards zero, indicating a fully de-localized state. \DIFdelbegin %DIFDELCMD < {%%%
\DIFdelend Contrast this with the IPR plots shown in the bottom left panel of fig\DIFdelbegin %DIFDELCMD < }%%%
\DIFdelend \DIFaddbegin \DIFadd{.}\DIFaddend ~\ref{fig:ipr:isinglowfrk} {for the integrable TFIM}.  \DIFdelbegin %DIFDELCMD < {%%%
\DIFdelend The top panel of fig.\DIFdelbegin %DIFDELCMD < }%%%
\DIFdelend ~\ref{fig:phase_crossover} {also reveals} a gradual increase {in the unity} {towards unity} of IPR over a certain frequency range, specifically at $\omega \approx 5$. In addition, the rise does not cross with those for different values of $N$, suggesting the onset of a phase crossover~\cite{sierant_2023, sachdev_quantum_2011}. As the size of the system increases, the crossover region becomes smoother, rather than sharper.  \DIFaddbegin \DIFadd{The crossover frequency $\omega_c$ have been estimated from the IPR data and plotted for different system sizes $N$ in the inset of fig.~\ref{fig:phase_crossover}. It is observed that $\omega_c$ rises sharply as $N$ increases past $40$. This indicates that $\omega_c$ blows up in the thermodynamic limit, where $N \to \infty$.
}\DIFaddend \begin{figure}[t!]
	\centering
	\DIFdelbeginFL %DIFDELCMD < \includegraphics[width =8.5cm]{phase_crossover_LMG.jpeg}
%DIFDELCMD < 		%%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width =8.5cm]{figure_9.jpeg}
	\DIFaddendFL \caption{\DIFdelbeginFL %DIFDELCMD < {%%%
\DIFdelendFL In the top panel, the IPR of the \DIFdelbeginFL \DIFdelFL{lowest quasienergy }\DIFdelendFL Floquet state \DIFaddbeginFL \DIFaddFL{with the lowest quasienergy }\DIFaddendFL is plotted as a function of \DIFdelbeginFL %DIFDELCMD < } %%%
\DIFdelendFL $\omega$ \DIFdelbeginFL %DIFDELCMD < {%%%
\DIFdelendFL for different \DIFdelbeginFL %DIFDELCMD < } %%%
\DIFdelendFL $N$s, \DIFdelbeginFL %DIFDELCMD < { %%%
\DIFdelendFL with amplitude $h$ adjusted to ensure that \DIFdelbeginFL %DIFDELCMD < } %%%
\DIFdelendFL $J_0(\eta)=0$ \DIFdelbeginFL %DIFDELCMD < {%%%
\DIFdelendFL always.The \DIFaddbeginFL \DIFaddFL{data points are shown, together with sigmoid curves $\phi_{IPR}=\left[1+e^{-\alpha\left(\omega-\omega_c\right)}\right]^{-1}$ that were fitted to each dataset. The }\DIFaddendFL smooth rise in IPR defines a phase crossover (top panel) between a fully thermal phase to a fully localized phase. \DIFaddbeginFL \DIFaddFL{In the inset, the crossover frequency $\omega_c$ is plotted against $N$, and  is found to rise sharply with N. }\DIFaddendFL The bottom panel plots the IPR versus \DIFdelbeginFL %DIFDELCMD < } %%%
\DIFdelendFL $N$ \DIFdelbeginFL %DIFDELCMD < {%%%
\DIFdelendFL for small \DIFdelbeginFL %DIFDELCMD < } %%%
\DIFdelendFL $\omega$s, \DIFdelbeginFL %DIFDELCMD < {%%%
\DIFdelendFL also with \DIFdelbeginFL %DIFDELCMD < } %%%
\DIFdelendFL $h$ \DIFdelbeginFL %DIFDELCMD < {%%%
\DIFdelendFL adjusted. The curves asymptotically approach the dependency \DIFdelbeginFL %DIFDELCMD < } %%%
\DIFdelendFL $\phi_{IPR}\sim 1/N$\DIFdelbeginFL %DIFDELCMD < {%%%
\DIFdelendFL , indicating thermalization at low-\DIFdelbeginFL %DIFDELCMD < }%%%
\DIFdelendFL $\omega$.}
...
\DIFdelendFL \DIFaddbeginFL \begin{figure}[ht!]
	\DIFaddendFL \centering
	\DIFdelbeginFL %DIFDELCMD < \includegraphics[width = 9.cm]{hbar_avg_std.jpeg}
%DIFDELCMD < 	%%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width = 8.cm]{figure_10.jpeg}
	\DIFaddendFL \caption{The \DIFaddbeginFL \DIFaddFL{temporal }\DIFaddendFL standard deviation of the heating rate, denoted by $\expval{H}_{std}$, calculated over a span of $t=500 T$ for two system sizes. Here, $h$ is varied to keep $\eta = 4h/\omega$ at the first root of $J_0(\eta)$. \DIFdelbeginFL \DIFdelFL{A nonsingular rise has been identified at $\omega \approx 4.0$. $\expval{H}_{std}$ exhibits a diminutive magnitude below that value of $\omega$, while a linear rise is observed at higher frequencies, consistent with freezing. A vanishingly small standard deviation for $\omega \ll 4.0$ indicates }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{In }\DIFaddendFL the \DIFdelbeginFL \DIFdelFL{presence of a thermal region}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{inset}\DIFaddendFL , \DIFdelbeginFL \DIFdelFL{whereas a larger finite standard deviation suggests the existence of athermal behaviour. The small peaks observed }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$\expval{H}_{std}$ is plotted against system size }\DIFaddendFL at \DIFdelbeginFL \DIFdelFL{$\omega\in [2, 4)$ are finite-size effects that disappear in the thermodynamic limit}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{low-frequency ($\omega=0.7$)}\DIFaddendFL .}
	\label{fig:havg_std}
\end{figure}

We can also look at this crossover more clearly in the plots of the heating rate of the system, defined simply by the expectation value of the Hamiltonian, $\expval{\hat{H}(t)}$. We have carried out the numerical evaluation from the simulated dynamics over $t=500 T$. When the system is adequately described by FETH, the temporal fluctuations in the {Hamiltonian}, defined by $\expval{H}^2_{std}\equiv \overline{\expval{\hat{H}}^2}-\overline{\expval{\hat{H}}}^2$ (see \DIFdelbegin \DIFdel{eqn}\DIFdelend \DIFaddbegin \DIFadd{eq.}\DIFaddend ~\ref{eq:lt_avg}), \DIFdelbegin \DIFdel{are minimal }\DIFdelend \DIFaddbegin \DIFadd{approach $1/8$ }\DIFaddend in the thermodynamic limit\DIFdelbegin \DIFdel{, as the spread of states leads to a limited standard deviation\mbox{%DIFAUXCMD
\cite{reimann_symmetry-prohibited_2021}}\hspace{0pt}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{~}\footnote{\DIFadd{Thermodynamically, at $\beta\equiv 1/k_BT=0$, we have $\expval{H}^2_{std}=\Tr{H^2_0}/2^N$. Substituting eqn.~\ref{eq:h0h1} for $H_0$ yields $1/8$ on the RHS.}}\DIFaddend . Conversely, the onset of athermality is indicated by nonzero fluctuations in time. If we set the initial state to the fully polarized state in the TSS (given by $\ket{s_N}$), then the 
onset of freezing, together with DMBL, will result in nearly infinite hysteresis in the ensuing dynamics, causing $\ket{\psi}(t)\approx \ket{s_N} \forall t$. From \DIFdelbegin \DIFdel{eqn}\DIFdelend \DIFaddbegin \DIFadd{eq}\DIFaddend .~\ref{eq:driven:ham}, we can clearly see that this will lead to a linearly rising dependence on $\omega$ in $\expval{H}_{std}$ as long as we stick to a \DIFdelbegin %DIFDELCMD < {%%%
\DIFdelend localization point given by a fixed \DIFdelbegin %DIFDELCMD < } %%%
\DIFdel{$h/\omega$~}\DIFdelend \DIFaddbegin \DIFadd{$4h/\omega$~}\DIFaddend \footnote{When frozen, $\ket{\psi(t)}\approx \ket{s_N}$. From eqn\DIFaddbegin \DIFadd{.}\DIFaddend ~\ref{eq:driven:ham}, $\expval{\hat{H}_{0,1}}$ are both approx. constant. Averaging the square of $\expval{\hat{H}(t)}$ over long times yields dependency  $\sim h^2 + \delta$, where $\delta\sim h_0\ll 1$. Thus, the std. devn. in time $\sim h\sim \omega$, since $\eta=4h/\omega$ is kept fixed.}. All these observations are corroborated by the \DIFdelbegin \DIFdel{heating rate plots in figure}\DIFdelend \DIFaddbegin \DIFadd{plots in fig.~\ref{fig:havg_std}, where we have displayed the temporal standard deviation of the heating rate for increasing $\omega$, while engineering $h$ such that the system is always at a localization point. A nonsingular rise has been identified at $\omega \approx 4.0$. The heating rate exhibits diminutive fluctuations below that value of $\omega$, consistent with thermalization, while a linear rise is observed at higher frequencies, consistent with actual localization. When $\omega \ll 4.0$, the diminutive standard deviation asymptotically approaches the theoretical value of $0.125$ in the thermodynamic limit, as can be seen in the inset of fig.}\DIFaddend ~\ref{fig:havg_std}. \DIFaddbegin \DIFadd{The small peaks observed at $\omega\in [2, 4)$ are finite-size effects that disappear in the thermodynamic limit. In the inset, $\expval{H}_{std}$ is plotted against system size at low-frequency ($\omega=0.7$) and $h$ engineered to lie at a localization point.
}


\DIFaddend \section{\label{sec:level7}Conclusion and Outlook}
We have delved into the onset of freezing and phase cross-over in 1D spin systems driven by a \DIFdelbegin \DIFdel{time-periodic }\DIFdelend \DIFaddbegin \DIFadd{time periodic }\DIFaddend transverse field, contrasting the responses in the Transverse Field Ising Model (TFIM) with that of the long-range Lipkin-Meshkov-Glick Model (LMG). The parametrization of DMBL is based on the Inverse Participation Ratio (IPR) of the Floquet eigenstates. Our investigations compared the IPRs from both models numerically, and found the emergence of thermal behavior at low frequencies and freezing at high frequencies for the LMG model, the latter \DIFaddbegin \DIFadd{being }\DIFaddend a direct consequence of the appearance of additional approximately conserved quantities.

Long-range spins exhibit strong localization in spin-coordinate space for the LMG model when the drive frequency is $\omega \gg J$, where $J$ represents the spin exchange energy. The localization of the LMG model occurs at specific \DIFdelbegin \DIFdel{resonance points }\DIFdelend \DIFaddbegin \DIFadd{freezing points in the space }\DIFaddend of the drive frequency $\omega$ and amplitude $h$, at $J_0(4h/\omega)=0,\; \omega \gg J$. This is apparently similar to the phenomenon of Dynamical Freezing (DMF) in the Transverse Field Ising Model (TFIM)\DIFaddbegin \DIFadd{, as well as DMBL in non-integrable short-range models}\DIFaddend . Plots of the IPR for a range of frequencies along the \DIFdelbegin \DIFdel{resonance }\DIFdelend \DIFaddbegin \DIFadd{freezing }\DIFaddend manifold exhibits a smooth increase in IPR yielding a quantum phase-crossover from a thermal phase governed by the Floquet Eigenstate Thermalization Hypothesis (FETH) to a Dynamically Many-Body localized phase (DMBL). \DIFdelbegin \DIFdel{Thus}\DIFdelend \DIFaddbegin \DIFadd{However, in sharp contrast to the phase transitions seen in short-range models, the crossover frequency $\omega_c$ diverges with the system size, indicating that DMBL is unstable in the thermodynamic limit for finite drive frequencies. Nonetheless}\DIFaddend , the suppression  of thermalization through  Dynamical Many Body Localization in \DIFaddbegin \DIFadd{finite sized }\DIFaddend long-range systems can be controlled via Floquet engineering, even in clean systems without any disorder.	Thus, periodically driven long-range spin systems are an excellent tool for investigating disorder-free Many Body Localization, as can be readily seen via the IPR of its Floquet modes.


There are several unexplored indicators of DMBL, such as entanglement entropy and level statistics~\cite{khemani_phase_2016}, which we defer to future studies. In addition, Halpern in 2019 proposed a quantum engine based on MBL\cite{yunger_halpern_quantum_2019} which works  between strong localized and thermal phases of the system. In our proposed LMG model, tuning the system parameters by bringing them to the \DIFdelbegin \DIFdel{resonance }\DIFdelend \DIFaddbegin \DIFadd{freezing }\DIFaddend points, then adiabatically cycling the frequency from the thermal region to the DMBL region, can achieve a similar engine without going through a phase transition. 

\DIFdelbegin %DIFDELCMD < \subsection {%%%
\DIFdel{Acknowledgements:}%DIFDELCMD < }
%DIFDELCMD < 	%%%
...
\DIFaddbegin \DIFadd{AR thanks Prof. Arnab Das, School of Physical Sciences, Indian Association for the Cultivation of Science, Kolkata, for useful suggestions.
}\end{acknowledgments}
\DIFaddend 

%\nocite{*}
\bibliographystyle{apsrev4-2}
\DIFdelbegin %DIFDELCMD < \bibliography{main110823}%%%
\DIFdelend \DIFaddbegin \bibliography{main}\DIFaddend % Produces the bibliography via BibTeX.
