...
\begin{document}
	...
	\begin{figure}[ht!]
		\centering
		\includegraphics[width =8cm]{corrN8sz0sz3avg_tfim_sx_xxz_sx.pdf}
		\caption{Average spin-spin correlation $\expval{S^z_0 S^z_3}$ for a 1D spin-1/2 chain with periodic boundary conditions. The chain consists of $N=8$ spins and is governed by the $TFIM+S_x$ model (Panel-A) as described by the Hamiltonian in Eq.~\ref{eq:tfim_sx}. Panel-B shows the XXZ model described by Eq.~\eqref{eq:xxzsx}. After starting from the fully spin-polarized state for systems at $t=0$, the systems were evolved for a long time $t=300T$ with a high frequency $\omega=90$. Following this, the spin correlation averages were computed.  The vertical dashed lines represent the roots of $\mathcal{J}_0$. In panel- A, the spin-spin correlation is suppressed at lower roots of $\mathcal{J}_0\left(2h/\omega\right)$, but it becomes close to one at higher roots for $TFIM+S_x$. For the $XXZ$ model, the spin-correlation remains constant for all values of $h/\omega$. However, the introduction of an additional $S_x$ results in a localization at only roots of $\mathcal{J}_0\left(h/\omega\right)$ and delocalization away from it as shown in Panel-B.
		}
	...
	%\label{fig:xxzsx}
	%\end{figure}
	\red{Interestingly, if we apply this approach to investigate the periodically driven XXZ model, we notice that the system remains frozen for all drive parameters. The XXZ Hamiltonian, including a longitudinal field, is given by
	\begin{multline}
	...
	\label{eq:xxzsxmov}
	\end{equation}
	where the moving frame XXZ Hamiltonian
	\begin{align}
		H_{_{XXZ}}^{mov} =& \frac12 \sum_i \Bigg\{ J \Big[ \hat{\sigma}^x_i \hat{\sigma}^x_{i+1} \cos[2](\zeta) + \hat{\sigma}^y_i \hat{\sigma}^y_{i+1} \sin[2](\zeta)\nonumber\\ 
		&+\frac12\left(\hat{\sigma}^x_i \hat{\sigma}^y_{i+1} + \hat{\sigma}^y_i \hat{\sigma}^x_{i+1}\right)\sin(2\zeta)\Big]\nonumber \\
		 &+ J \Big[ \hat{\sigma}^x_i \hat{\sigma}^x_{i+1} \sin[2](\zeta) + \hat{\sigma}^y_i \hat{\sigma}^y_{i+1} \cos[2](\zeta)\nonumber\\
		  &-\frac12\left(\hat{\sigma}^x_i \hat{\sigma}^y_{i+1} + \hat{\sigma}^y_i \hat{\sigma}^x_{i+1}\right)\sin(2\zeta)\Big]+\Delta \sum_{i}  \hat{\sigma}_{i}^{z} \hat{\sigma}_{i+1}^{z}\Bigg\}\nonumber\\
		  =& \frac12 \sum_i \Bigg\{J\Big(\hat{\sigma}^x_i \hat{\sigma}^x_{i+1}  + \hat{\sigma}^y_i \hat{\sigma}^y_{i+1} \Big) + \Delta \sum_{i}  \hat{\sigma}_{i}^{z} \hat{\sigma}_{i+1}^{z}\Bigg\},
	\end{align}
	and $\zeta \equiv \frac{h}{\omega}\sin(\omega t)$. We consider fermionic creation and annihilation operator, $\hat{\sigma}^+$ and $\hat{\sigma}^-$ respectively and transforming moving frame Hamiltonian in their terms,
	\begin{align}
	...
	\end{align} 
	where, $\hat{\sigma}^+_i = 1/2(\hat{\sigma}^x_i + i\hat{\sigma}^y_i)$ and $\hat{\sigma}^-_i = 1/2(\hat{\sigma}^x_i - i\hat{\sigma}^y_i)$. The first two terms in the above eqn. annihilates together any potential combinations of states of spins. However, if we populate the system with fully polarized state, which is one of the eigen states of $\displaystyle \sum_i \hat{\sigma}^z_i \hat{\sigma}^z_{i+1}$, the contribution from $\displaystyle \sum_i \hat{\sigma}^z_i \hat{\sigma}^z_{i+1}$ gets the moving frame Hamiltonian frozen under periodic drive. Now, if a longitudinal field is incorporated in XXZ model as described in Eq.~\eqref{eq:xxzsxmov}, it will add single particle dynamics to the XXZ Hamiltonian. Next, we apply RWA in Eq.~\ref{eq:xxzsxmov} and smoothing out all harmonic oscillations from the Jacobi Anger expansion yields
	\begin{align}
	&H_{_{XXZ+S_{X}}}^{_{RWA}} = H_{_{XXZ}}^{RWA} +  \frac12 \mathcal{J}_0 \left(\frac{h}{\omega}\right)\sum_i \hat{\sigma}^x_i,\nonumber\\
	&H_{_{XXZ}}^{RWA} =\frac12 \sum_i J\left( \hat{\sigma}^x_i \hat{\sigma}^x_{i+1} + \hat{\sigma}^y_i \hat{\sigma}^y_{i+1} \right) + \Delta \hat{\sigma}_{i}^{z} \hat{\sigma}_{i+1}^{z}.
	\label{eq:xxz_sx_rwa}
	\end{align}
	Thus freezing can occur in XXZ model regardless of the drive parameters. However when the $\hat{\sigma}^x-$ field is introduced, freezing can occur when the drive parameters are controlled in such manner that ($h/\omega$) lies on any root of the Bessel function.}
