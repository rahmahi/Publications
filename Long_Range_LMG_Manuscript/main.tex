...
\begin{document}
	...
	Periodically driven Quantum Many Body Systems can experience Dynamical Freezing (DMF) when  dynamical hysteresis stops observables from reaching their diagonal averaged values and thermalizing to infinite temperature ~\cite{bordia_periodically_2017, sahoo_periodically_2019, das_exotic_2010}. Under certain resonance conditions in the drive parameters, DMF can cause the response to ‘freeze’ completely to its initial value at all times. This arises as a consequence of additional approximate symmetries that occur at resonance. DMF has been demonstrated via the Rotating Wave Approximation (RWA) in the driven TFIM with nearest neighbour interactions ~\cite{mbeng_quantum_2020} and is shown to be protected when translational invariance is explicitly broken (say, by disorder) ~\cite{yamada_localization_2022, roy_fate_2015}. The application of disorder in an integrable system has been identified as a crucial component in the formation of the Many-Body Localization phase. This occurs when the effective many-body Hamiltonian at higher symmetry vanishes. An alternative approach to realizing localization in many-body system involves the introduction of dynamical periodic driving in a clean, non-integrable system, as opposed to the conventional method of introducing disorder.
	
	The utilization of Floquet theory is a viable approach for the analysis of time-dependent systems in the field of quantum mechanics. The Floquet Hamiltonian is a useful tool for converting a Hamiltonian that is time-dependent into a Hamiltonian that is time-independent. This is achieved by the introduction of an effective time-independent operator. The Floquet modes are characterized as eigenstates of the Floquet Hamiltonian that are independent of time, and are obtained as solutions of the Floquet equation. Nonetheless, the temporal progression of the system manifests a temporal variability via a phase coefficient that captures the cyclic stimulation of the system. The temporal aspect holds significant importance in the analysis of the behavior of time-varying systems using the principles of Floquet theory.
...
Consider a many-body quantum system described by a time-dependent periodic Hamiltonian H(t) with a period T. The Schrödinger equation governing the time evolution of the system is given by $i\hbar \pdv{\psi(t)}{t} = \hat{H}(t) \ket{\psi(t)}$, where $\ket{\psi(t)}$ is the state vector of the system at time t. We aim to derive ETH by examining the expectation value of an observable $\hat{A}$ in an eigenstate of the Hamiltonian.
