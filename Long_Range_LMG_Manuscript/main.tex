...
\begin{document}
	...
	%\label{fig:xxzsx}
	%\end{figure}
	\red{Interestingly, if we apply this approach to investigate the periodically driven XXZ model, we notice that the system remains frozen for all drive parameters \sout{in the RWA limit}. The XXZ Hamiltonian, including a longitudinal field, is given by
	\begin{multline}
	...
	\end{align}
	and $\zeta \equiv \frac{h}{\omega}\sin(\omega t)$. We consider fermionic creation and annihilation operator, $\hat{\sigma}^+$ and $\hat{\sigma}^-$ respectively and transforming in their terms,
	\begin{align}
	...
	\end{align} 
	where, $\hat{\sigma}^+_i = 1/2(\hat{\sigma}^x_i + i\hat{\sigma}^y_i)$ and $\hat{\sigma}^-_i = 1/2(\hat{\sigma}^x_i - i\hat{\sigma}^y_i)$. The first two terms in the above eqn. together annihilates any possible combinations of states of spins. However, if we populate the system with fully polarized state, which is one of the eigen state of $\displaystyle \sum_i \hat{\sigma}^z_i \hat{\sigma}^z_{i+1}$. This gets the $H_{_{XXZ}}^{mov}$ frozen under periodic drive. Now, if a longitudinal field is introduced in XXZ model as in Eq.~\eqref{eq:xxzsxmov}, it will add single particle dynamics to the system. Now, we apply RWA in Eq.~\ref{eq:xxzsxmov} and smoothing out all harmonic oscillations from the Jacobi Anger expansion yields
	\begin{align}
	...
	\end{align}
	In the absence of the $\hat{\sigma}^x-$field, freezing occurs irrespecive of the druive parameters, as 	
	Freezing can occur when the drive parameters are engineered such that ($h/\omega$) lies on any root of the Bessel function.}
