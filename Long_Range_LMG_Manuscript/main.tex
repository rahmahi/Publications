...
\label{eq:tfim_sx}
\end{equation}	
RWA reduces eqn.\eqref{eq:tfim_sx} to,
\begin{multline}
	...
	\label{eq:tfim_sx1}
\end{multline}
\blue{If drive parameters $h$ and $\omega$ are controlled such a way that the ratio $\left(\frac{2h}{\omega}\right)$ lies at one of the roots of Bessel function, then the first part of eqn.\eqref{eq:tfim_sx1} vanishes yet the second term survives. The second term incorporates non interacting spins which is localized along a set of  basis vector corresponding to spin-spin interaction axis. However if additional spin-spin interaction term is added, then this phenomenon can not be replicated, as an example in XXZ model.} 
