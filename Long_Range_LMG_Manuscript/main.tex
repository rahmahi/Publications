...
\begin{document}
	...
	%\label{fig:xxzsx}
	%\end{figure}
	\red{Interestingly, if we apply this approach to investigate the periodically driven XXZ model, we notice that the system remains frozen for all drive parameters in the RWA limit.  The XXZ Hamiltonian, including a longitudinal field, is given by
	\begin{multline}
	...
	\begin{equation}
	H_{_{XXZ+S_{X}}}^{_{mov}} =H_{_{XXZ}}^{mov} +  ,
	\label{eq:xxzsxmov}
	\end{equation}
	where the moving frame XXZ Hamiltonian
	\begin{multline}
	H_{_{XXZ}}^{mov} = \frac12 \sum_i \Bigg\{ J \Big[ \hat{\sigma}^x_i \hat{\sigma}^x_{i+1} \cos[2](\zeta) + \hat{\sigma}^y_i \hat{\sigma}^y_{i+1} \sin[2](\zeta)+\\ \frac12\left(\hat{\sigma}^x_i \hat{\sigma}^y_{i+1} + \hat{\sigma}^y_i \hat{\sigma}^x_{i+1}\right)\sin(2\zeta)\Big] \\
	 ...
	 +{\Delta \sum_{i}  \hat{\sigma}_{i}^{z} \hat{\sigma}_{i+1}^{z}}\Bigg\},
	\end{multline}
	and $\zeta \equiv \frac{h}{\omega}\sin(\omega t)$. Applying the RWA in Eq.~\ref{eq:xxzsxmov} by and smoothing out all harmonic oscillations from the Jacobi Anger expansion yields
	\begin{align}
	...
	In the absence of the $\hat{\sigma}^x-$field, freezing occurs irrespecive of the druive parameters, as 
	 ...
	 Freezing can occur when the drive parameters are engineered such that 
	 ($h/\omega$) lies on a root of the Bessel function. However, when the ratio $h/\omega$ departs from the roots of $\mathcal{J}_0$, an instance of delocalization takes place.}
