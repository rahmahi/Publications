...
\begin{multline}
	...
	\label{eq:tfim_sx1}
\end{multline}
\blue{If drive parameters $h$ and $\omega$ are controlled such a way that $\left(\frac{2h}{\omega}\right)$ lies at one of the roots of Bessel function, then the first part of Eq.\eqref{eq:tfim_sx1} vanishes yet the second term survives. The second term consists of non interacting spins for which FETH-IPR can be localized along set of  basis vector corresponding to spin-spin interaction. However if we add additional spin-spin interactions, then this phenomenon can not be replicated, as an example in XXZ model.} 
